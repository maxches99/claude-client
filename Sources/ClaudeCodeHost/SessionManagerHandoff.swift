...
/// A server has no desk to hand a session to: nothing is offered, and asking says why.
extension SessionManager {
    public func handoffTargets(sessionId: String) -> [HandoffTarget] { [] }

    public func handoff(sessionId: String, targetId: String) async throws {
        throw GitError.refused("This host has no desktop to continue on — open the session on a Mac.")
    }
}
#endif
